...
def self_consistency(R: dict, clips: list[str]) -> dict:
    ...
    for m in METHODS + ["frozen"]:
        ...
        for c in clips:
            ...
            flick.append(mm.get("flicker", {}).get("mean_abs_change", float("nan"))
                         if isinstance(mm.get("flicker"), dict) else float("nan"))
        if vals:
            out[m] = {"iou_consecutive": mean(vals), "clips": len(vals)}
            if acc:
